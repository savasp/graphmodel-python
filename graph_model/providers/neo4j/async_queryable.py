...
from typing import Any, AsyncIterator, Callable, List, Optional, Type, TypeVar, cast
...
from ...querying.async_streaming import IAsyncGraphQueryable
...
class Neo4jAsyncNodeQueryable(IAsyncGraphQueryable[TNode]):
    """
    Neo4j async queryable for node queries.

    Provides async streaming capabilities for Neo4j node queries.
    """
    ...
    def __init__(
        ...
        self._predicate: Optional[Callable[[TNode], bool]] = None
    ...
    async def _execute_streaming(self) -> AsyncIterator[TNode]:
        """Execute the streaming query and yield results."""
        query = self._build_streaming_query()
        result = await self._session.run(cast(LiteralString, query.query), query.parameters)

        count = 0
        async for record in result:
            # Apply in-memory predicate if set
            if self._predicate is not None:
                # Deserialize node for predicate evaluation
                node_data = record.get("n") or record
                if node_data is None:
                    continue
                node = cast(TNode, self._serializer.deserialize_node(node_data, self._node_type))
    ...
    async def single_async(self, predicate: Optional[Callable[[TNode], bool]] = None) -> TNode:
        ...
        assert found is not None  # Type checker assertion
        return found
    ...
    async def count_async(self, predicate: Optional[Callable[[TNode], bool]] = None) -> int:
        ...
        if predicate is None:
            ...
            result = await self._session.run(cast(LiteralString, count_query.query), count_query.parameters)
    ...
    async def any_async(self, predicate: Optional[Callable[[TNode], bool]] = None) -> bool:
        ...
        if predicate is None:
            ...
            result = await self._session.run(cast(LiteralString, exists_query.query), exists_query.parameters)
            record = await result.single()
            if record is None:
                return False
    ...
    def select_async(self, selector: Callable[[TNode], Any]) -> "Neo4jAsyncProjectionQueryable":
...
class Neo4jAsyncProjectionQueryable(IAsyncGraphQueryable[Any]):
    ...
    def __init__(
        ...
        self._source_queryable = Neo4jAsyncNodeQueryable(
            cast(Type[INode], source_type), session, serializer, batch_size
        )
