...
from graph_model.core.graph import (
    IGraph,
    IGraphNodeQueryable,
...
from .serialization import Neo4jSerializer, SerializedNode
...
class Neo4jGraph(IGraph[TNode, TRelationship]):
    ...
    def __init__(self) -> None:
        ...
        self._traversal_executor: Optional[Neo4jTraversalExecutor] = None  # Lazy initialization
        self._aggregation_executor: Optional[Neo4jAggregationExecutor] = None  # Lazy initialization

    def _get_traversal_executor(self, session: Any) -> Neo4jTraversalExecutor:
    ...
    def _get_aggregation_executor(self, session: Any) -> Neo4jAggregationExecutor:
    ...
    async def update_node(
        ...
        try:
            ...
            try:
                ...
                if not record:
                    raise GraphError(f"Node with ID {node.id} not found")
    ...
    async def update_relationship(
        ...
        try:
            ...
            try:
                ...
                if not record:
                    raise GraphError(f"Relationship with ID {relationship.id} not found")
